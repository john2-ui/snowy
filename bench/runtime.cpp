...
int main(int argc, char** argv) {
    try {
        ...
        if (argc > 3 || (mode != "task" && mode != "schedule" && mode != "spawn"))
            throw std::invalid_argument("usage: snowy_runtime [task|schedule|spawn] [count]");
        std::vector<double> samples;
        for (unsigned sample = 0; sample < 10; ++sample) {
            snowy::loop loop;
            std::uint64_t sum = 0;
            auto work = mode == "task" ? tasks(count, sum)
                : mode == "schedule" ? yields(loop, count) : spawn(loop, count);
            loop.spawn(std::move(work));
            const auto start = bench::clock::now();
            loop.run();
            const double ns = bench::elapsed(start) / count;
            if (mode == "task" && sum != std::uint64_t{count} * (count - 1) / 2)
