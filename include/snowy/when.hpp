...
struct group {
    ...
    group(loop& loop, std::size_t size) : done(loop), values(size) {}
};
...
template <typename T>
task<> collect(task<T> input, group<T>& state, std::size_t index, bool race) {
    std::exception_ptr error;
    try {
...
task<std::vector<detail::result<T>>> when_all(loop& loop, std::vector<task<T>> tasks) {
    ...
    for (std::size_t i = 0; i < tasks.size(); ++i) {
        ++state.pending;
        try { loop.spawn(detail::collect(std::move(tasks[i]), state, i, false)); }
...
task<std::pair<std::size_t, detail::result<T>>> when_any(
    ...
    for (std::size_t i = 0; i < factories.size(); ++i) {
        ++state.pending;
        try { loop.spawn(detail::collect(factories[i](state.stop.get_token()), state, i, true)); }
        catch (...) {
            --state.pending;
...
/** @brief Apply a cooperative deadline; return only after canceled work is drained.
...
task<T> timeout(loop& loop, loop::clock::duration delay,
                std::function<task<T>(std::stop_token)> factory) {
    std::vector<std::function<task<T>(std::stop_token)>> jobs;
    jobs.push_back(std::move(factory));
    jobs.push_back([&loop, delay](std::stop_token token) -> task<T> {
        co_await loop.sleep(delay, token);
        throw std::system_error(std::make_error_code(std::errc::timed_out));
    });
    auto result = co_await when_any<T>(loop, std::move(jobs));
